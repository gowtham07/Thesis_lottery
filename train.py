...
def train_eval_loop(model, loss, optimizer, scheduler, train_loader, test_loader, device, epochs, verbose,l,sparsity):
...
def post_prune_train_eval_loop(model, loss, optimizer, scheduler, train_loader, test_loader, device, epochs, verbose,l, sparsity,res_dir):
    epoch_div = epochs//3
    epoch_1 = 0
    epoch_2 = epoch_1 +epoch_div
    epoch_3 = epoch_2 + epoch_div 
    #rows = [[np.nan, test_loss, accuracy1, accuracy5]]  
    for epoch in tqdm(range(epochs)):
        train_loss = train(model, loss, optimizer, train_loader, device, epoch, verbose)
        if epochs == epoch_1:
             torch.save(model.state_dict(),"{}/last_prune_level_epoch{}_model.pt".format(res_dir,epoch_1))
        if epochs == epoch_2:
             torch.save(model.state_dict(),"{}/last_prune_level_epoch{}_model.pt".format(res_dir,epoch_2))
        if epochs == epoch_3:
             torch.save(model.state_dict(),"{}/last_prune_level_epoch{}_model.pt".format(res_dir,epoch_3))          
        #row = [train_loss, test_loss, accuracy1, accuracy5]
        scheduler.step()
    test_loss, accuracy1, accuracy5 = eval_1(model, loss, test_loader, device, verbose,l,sparsity)